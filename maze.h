...
public:
    ...
    lateUpdateReq(std::atomic<float>* d1_p, float d1_push, short parentX, short parentY, bool HV) {
        this->d1_p = d1_p;
        this->d1_push = d1_push;
...
using OBIM_late = gwl::OrderedByIntegerMetric<decltype(RequestIndexerLate), PSChunk>;
//using OBIM_concurrent = gwl::OrderedByIntegerMetric<decltype(RequestIndexerConcurrent), PSChunk>;
...
void setupHeap(int netID, int edgeID, local_pq& pq1, local_vec& v2, int regionX1, 
    int regionX2, int regionY1, int regionY2, float** d1, int** corrEdge, bool** inRegion)
{
...
void updateRouteType1(TreeNode *treenodes, int n1, int A1, int A2, int E1x, int E1y, TreeEdge *treeedges, int edge_n1A1, int edge_n1A2)
{
    ...
    int gridsX_n1A1[2 * (xGrid+yGrid)], gridsY_n1A1[2 * (xGrid+yGrid)], gridsX_n1A2[2 * (xGrid+yGrid)], gridsY_n1A2[2 * (xGrid+yGrid)];
...
void updateRouteType2(TreeNode *treenodes, int n1, int A1, int A2, int C1, int C2, int E1x, int E1y, TreeEdge *treeedges, int edge_n1A1, int edge_n1A2, int edge_C1C2)
{
    ...
    int gridsX_n1C1[2 * (xGrid+yGrid)], gridsY_n1C1[2 * (xGrid+yGrid)];
    int gridsX_n1C2[2 * (xGrid+yGrid)], gridsY_n1C2[2 * (xGrid+yGrid)];
    ...
    //galois::substrate::PerThreadStorage<THREAD_LOCAL_STORAGE>* thread_local_storage)
{
    ...
    //cout << " i = vCap:" << v_costTable[vCapacity-1] << " " << v_costTable[vCapacity] << " " << v_costTable[vCapacity+1] << endl;
   ...
   //galois::runtime::profileVtune( [&] (void) {
			...
			if(treeedge->len > mazeedge_Threshold) // only route the non-degraded edges (len>0)
			{
				...
				if(enter)
				{
						...
						if(curY>regionY1)
						{
							...
							{	
								if (curY < regionY2 - 1) {
									...
									if (tmp_cost < curr_d1 + VIA
                                        && d1[tmpY][curX] > tmp_cost + v_costTable[v_edges[grid].usage+v_edges[grid].red+(int)(L*v_edges[grid].last_usage)]) {
					...
					while(d1[curY][curX]!=0) // loop until reach subtree1
					{
                        ...
                        /*if(cnt > 2000) {
                            cerr << "Y: " << curY <<" X:" << curX << " hyperH: " << hyperH[curY][curX];
                            cerr << " hyperV:" << hyperV[curY][curX] << " HV: " << HV[curY][curX];
                            cerr << " d1: " << d1[curY][curX] << endl;
                            cerr << " deadloop return!" << endl;
                            reInitTree(netID);
                            return;
                        }*/
    ...
    galois::loopname("net-level parallelism")
    );//galois::do_all
    ...
    */ // must be fixed after competetion
...
void mazeRouteMSMD_block(int iter, int expand, float costHeight, int ripup_threshold, int mazeedge_Threshold, Bool Ordering, int cost_type, 
    galois::InsertBag<int>* net_shuffle)
    //galois::substrate::PerThreadStorage<THREAD_LOCAL_STORAGE>* thread_local_storage)
{
    //LOCK = 0;
                ...
                if(enter)
                {
                    ...
                    for(i=0; i<cnt_n1n2-1; i++)
                    {
                        if(gridsX[i]==gridsX[i+1]) // a vertical edge
                        {
                            ...
                            //v_edges[min_y*xGrid+gridsX[i]].usage += 1;
                            //galois::atomicAdd(v_edges[min_y*xGrid+gridsX[i]].usage, (short unsigned)1);
                            v_edges[min_y*xGrid+gridsX[i]].usage.fetch_add((short unsigned)1, std::memory_order_relaxed);
                        }
                        else ///if(gridsY[i]==gridsY[i+1])// a horizontal edge
                        {
                            min_x = min(gridsX[i], gridsX[i+1]);
                            //h_edges[gridsY[i]*(xGrid-1)+min_x].usage += 1;
                            //galois::atomicAdd(h_edges[gridsY[i]*(xGrid-1)+min_x].usage, (short unsigned)1);
                            h_edges[gridsY[i]*(xGrid-1)+min_x].usage.fetch_add((short unsigned)1, std::memory_order_relaxed);
                        }
                    }
                    /*if(LOCK){
                        for(i=0; i<cnt_n1n2-1; i++)
    ...
    galois::loopname("maze routing block")
    );//galois::do_all
    //}, "mazeroute vtune function");
    free(h_costTable);
    free(v_costTable);
...
int getOverflow2Dmaze( int* maxOverflow, int* tUsage)
{
    ...
    total_usage = 0;
	total_cap = 0;
	unsigned int dimx = xGrid;
    unsigned int dimy = yGrid;
...
//    fprintf(fpv, "\nVertical Congestion\n");
    ...
    for(i=0; i<yGrid-1; i++)
    {
        for(j=0; j<xGrid - 1; j++)
        {
            ...
            if(overflow > 0)
            {
                float red = (overflow >= 1)? 0 : (255 - ((float)overflow/1) * 255);
                int red_int = (int)red;
                image.set_pixel(j,i,255,(unsigned char)red_int,(unsigned char)red_int);
            }
            else
            {
...
void checkUsageCorrectness()
{
        ...
        for(edgeID=0; edgeID<num_edges; edgeID++)
        {
            ...
            if(treeedge->len > 0) // only route the non-degraded edges (len>0)
            {
                if(treeedge->route.type == MAZEROUTE)
                {
                    ...
                    //std::cout << "net: " << netID << " route lenth: " << treeedge->route.routelen << std::endl;

                    for(int i = 0; i < treeedge->route.routelen; i++)
                    {
                        //std::cout << "path: " << gridsX[i] << "," << gridsY[i] << "<->" << gridsX[i + 1] << "," << gridsY[i + 1] << std::endl;
    ...
    for(int i=0; i<yGrid; i++)
    {
        for(int j=0; j<xGrid-1; j++)
        {
            ...
            {
                diff++;
                //std::cout << "h edge diff: " << j << ", " << i << " check: " << hedge_usage[grid] << " actual: " << h_edges[grid].usage << std::endl;
    ...
    for(int i=0; i<yGrid-1; i++)
    {
        for(int j=0; j<xGrid; j++)
        {
            ...
            {
                diff++;
                //std::cout << "v edge diff: " << j << ", " << i << " check: " << vedge_usage[grid] << " actual: " << v_edges[grid].usage << std::endl;
            }
        }
    }
