...
//		printf("2pinnet l1 %d, l2 %d\n", l1, l2);
    	int layer1 = treenodes[n1].layer;
        d13D[layer1][y1][x1] = 0;
		directions3D[0][y1][x1] = ORIGIN;
        heap13D[0] = &(d13D[0][y1][x1]);
        *heapLen1 = 1;

        int layer2 = treenodes[n2].layer;
        d23D[layer2][y2][x2] = 0;
...
//			getLayerRange(treenodes, treeedges ,n1, edgeID, &topL, &botL);
			...
			//	cout << "msmd: " << treenodes[n1].x << " " << treenodes[n1].y << " " << treenodes[n1].botL << " " << treenodes[n1].topL << endl;
...
// get cur node from the queuehead
        ...
        if(n2<d) // n2 is a Pin node
        {
			...
			//if(string(nets[netID]->name) == "ionet11")
			//	cout << "msmd2: " << treenodes[n2].x << " " << treenodes[n2].y << " " << treenodes[n2].botL << " " << treenodes[n2].topL << endl;
				...
				if(newRipup3DType3(netID, edgeID))
				{
					...
					// re statis the node overlap
						...
						for(d=0;d<2*deg-2;d++)
						{
							...
							if(d<deg)
							{
								treenodes[d].botL = treenodes[d].topL   = treenodes[d].layer;
								//treenodes[d].l = 0;
