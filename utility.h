	...
	for (k = 0; k < routelen; k ++) {
		if (gridsX[k] == gridsX[k+1]) {
			...
			for (l = 0; l < numLayers; l ++) {
				...
				{
					cout << " intermediate netID: " << netID << " edgeID: " << edgeID << endl;
					cout << "cap = 0 : " << min_x << " " << gridsY[k] << " " << l << " " << " grid3D: " << grid << " gridH: " << gridH << endl;
				}*/
			}
		}
	}
	...
	/*for(netID=0;netID<numValidNets;netID++)
	{
		...
		if(string(nets[netID]->name) == "iopin242") {
			for(d=0;d<2*deg-2;d++)
			{
				if(d<deg)
				{
					cout << "pins: " << nets[netID]->pinX[d] << " " << nets[netID]->pinY[d] << " " << nets[netID]->pinL[d] << endl;
				}
			}
		}
	}*/
					...
					for(i=0; i<=treeedge->route.routelen; i++)
					{
						for (j = 0; j < i; j++) {
							if(gridsX[i]==gridsX[j] && gridsY[i]==gridsY[j]) // a vertical edge
							{
								...
								i = 0;
								j=0;
								printf("redundant edge component discovered: netID: %d x: %d y: %d\n", netID, gridsX[i], gridsY[i]);
							}
						}
...
void writeRoute3D(char routingfile3D[], parser::grGenerator grGen)
{
	...
	for(netID=0;netID<numValidNets;netID++)
	{
		...
		int grNetID = grGen.netName2grnetidx.find(nets[netID]->name)->second; 
		...
		for(int i = 0; i < deg; i++)
		{	
			//int high_layer = findLayer(int netID, TreeNode treenode)
			/*for (int layer = 0; layer <= nets[netID]->pinL[i] - 1; layer++) //this is not good, assuming all pins are on bot layer.
			{
				auto it = defDB.xGcellBoundaries.begin();
				std::advance(it, nets[netID]->pinX[i]);
				int llx = *it;

				it = defDB.yGcellBoundaries.begin();
				std::advance(it, nets[netID]->pinY[i]);
				int lly = *it;

				it = defDB.xGcellBoundaries.begin();
				std::advance(it, nets[netID]->pinX[i] + 1);
				int urx = *it;

				it = defDB.yGcellBoundaries.begin();
				std::advance(it, nets[netID]->pinY[i] + 1);
				int ury = *it;
			...
			for (int layer = nodes[i].botL; layer <= nodes[i].topL; layer++) //this is not good, assuming all pins are on botL layer.
			{
				...
				it = defDB.yGcellBoundaries.begin();
				std::advance(it, nodes[i].y);
				int lly = *it;

				it = defDB.xGcellBoundaries.begin();
				std::advance(it, nodes[i].x + 1);
				int urx = *it;

				it = defDB.yGcellBoundaries.begin();
				std::advance(it, nodes[i].y + 1);
				int ury = *it;
			...
			if (treeedge->len > 0) {
				...
				for(int output_cnt = 0; output_cnt < 1; output_cnt++)
				{
					for (i = 0; i <= routeLen; i ++) {
						...
						/*auto it = defDB.xGcellBoundaries.begin();
						std::advance(it, gridsX[i]);
						int llx = *it;

						it = defDB.yGcellBoundaries.begin();
						std::advance(it, gridsY[i]);
						int lly = *it;

						it = defDB.xGcellBoundaries.begin();
						std::advance(it, gridsX[i] + 1);
						int urx = *it;

						it = defDB.yGcellBoundaries.begin();
						std::advance(it, gridsY[i] + 1);
						int ury = *it;
						...
						fprintf(fp, "%d %d %d %d %s\n", llx, lly, urx, ury, layerName.c_str());
						if((layer + 1) * 2 < lefDB.layers.size())
						{
							fprintf(fp, "%d %d %d %d %s\n", llx, lly, urx, ury, lefDB.layers.at((layer + 1) * 2).name.c_str());
						}*/
						routedGrid.insert(Point3D(gridsX[i], gridsY[i], gridsL[i]));
						/*if((gridsL[i] + 1) * 2 < lefDB.layers.size())
	...
	for(netID=0;netID<numInvalidNets;netID++)
	{
		...
		if(netName == "net71037")
		{
			cout << "invalid net71037: " << endl;
			print = true;
		}

		if(grGen.netName2grnetidx.count(netName) == 0)
		{
			cout << "Error: unable to find net: " << netName << endl;
			exit(1);
		}
		int grNetID = grGen.netName2grnetidx.find(netName)->second; 
		auto grNet = grGen.grnets.at(grNetID);
		...
		int layer = invalid_nets[netID]->pinL[0] - 1;
		...
		if((layer + 2) * 2 < lefDB.layers.size())
		{
			fprintf(fp, "%d %d %d %d %s\n", llx, lly, urx, ury, lefDB.layers.at((layer + 2) * 2).name.c_str());
		}*/
