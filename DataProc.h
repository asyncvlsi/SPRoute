...
// Global variables
    ...
    int numValidNets, numInvalidNets; // # nets need to be routed (having pins in different grids)
    int numLayers;
	...
	int xcor[MAXD*2],ycor[MAXD*2], dcor[MAXD*2];
...
void readFile(char benchFile[])
{
    ...
    for(i=0; i<numNets; i++)
    {
        ...
        if (numPins<1000000) //for ICCAD 2019
        {
            pinInd = 0;
            for(j=0; j<numPins; j++)
            {
